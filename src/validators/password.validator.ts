...
const passwordValidator = (passwordName: string = 'Password') =>
  z
    ...
    .regex(
      /[A-Z]/,
      `${passwordName} must contain at least one uppercase letter`
    )
    .regex(
      /[a-z]/,
      `${passwordName} must contain at least one lowercase letter`
    )
